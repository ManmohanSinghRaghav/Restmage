...
router.post('/', auth, [
  ...
  body('propertyDetails.type').optional().isIn(['residential', 'commercial', 'industrial', 'mixed-use']),
  body('propertyDetails.dimensions.length').optional().isFloat({ min: 0 }),
  body('propertyDetails.dimensions.width').optional().isFloat({ min: 0 }),
  body('propertyDetails.dimensions.height').optional().isFloat({ min: 0 })
], createProject);
